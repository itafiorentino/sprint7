...
function App() {
  ...
  // append next api pages to shipsData
  useEffect(() => {
  ...
  return (
    <>
      <Navbar />
      <div className="App flex flex-col w-full min-h-screen items-center bg-black font-mono text-stone-200">
        <div name="ship-list-element" className="flex-row w-5/6 md:w-4/6 mt-4">
          {shipsData.map((ship) => (
            <ul key={ship.id} className=" bg-stone-900 p-4 mb-4">
        ...
        {isLoading && (
          <>
            <span className="loading loading-dots loading-md m-5"></span>
          </>
        )}
        <button
